...
ImmigrationEvent.EARLY_POPULATION_THRESHOLD = 12
...
ImmigrationEvent.nMinUndiscoveredRooms = -1
ImmigrationEvent.nMaxUndiscoveredRooms = -1
ImmigrationEvent.nMinExteriorRooms = -1
ImmigrationEvent.nMaxExteriorRooms = -1
ImmigrationEvent.bHostile = false
ImmigrationEvent.nChanceObey = 0.66
ImmigrationEvent.nChanceHostile = 0.00
ImmigrationEvent.sExpMod = 'population'
ImmigrationEvent.bAllowKillbots = false
ImmigrationEvent.bAttemptDock = false
...
function ImmigrationEvent.dialogTick(rController, tPersistentEventState, tTransientEventState, dt)
    ...
    if not tTransientEventState.tDialogStatus.tDlgSet then
        -- Ensure backwards compatibility for 1.07 era save files
        if not tPersistentEventState.tEventData then
            tPersistentEventState.tEventData = DFUtil.arrayRandom(EventData[tPersistentEventState.sEventType])
        end
        if not tPersistentEventState.sPortrait then
            tPersistentEventState.sPortrait = Portraits.getRandomPortrait()
        end

        tTransientEventState.tDialogStatus.tDlgSet = tPersistentEventState.tEventData
        tTransientEventState.tDialogStatus.sPortrait = tPersistentEventState.sPortrait
        ...
        rRequestUI:setTemplateUITexture('Picture',
                                        tTransientEventState.tDialogStatus.sPortrait,
                                        Portraits.PORTRAIT_PATH)
        local tReplacements= {
            Title = tDlgSet.title,
    ...
    elseif not tTransientEventState.tDialogStatus.bRequestUI then
        ...
        if not bSpawn and ignoreRefusal then
            bSpawn = true
            tReplacements = {
