...
CompoundEvent.nMinPopulation = 25
CompoundEvent.nMaxPopulation = -1
CompoundEvent.nMinTime = 60*30
CompoundEvent.nMaxTime = -1
CompoundEvent.nMinUndiscoveredRooms = -1
CompoundEvent.nMaxUndiscoveredRooms = -1
CompoundEvent.nMinExteriorRooms = -1
CompoundEvent.nMaxExteriorRooms = -1
CompoundEvent.bHostile = true
CompoundEvent.nChanceObey = 0.33
CompoundEvent.nChanceHostile = 1.00
CompoundEvent.sExpMod = 'population'
CompoundEvent.bAllowKillbots = true
CompoundEvent.bAttemptDock = false
...
function CompoundEvent.selectEvents(rController, tPersistentState, nPopulation, nElapsedTime, bMega)
    ...
    while nPoints > 0 or (not bMeteorStrike and bMega) do
        ...
        if tEvent.nNumSpawns and tEvent.tCharSpawnStats then
...
function CompoundEvent.dialogTick(rController, tPersistentEventState, tTransientEventState, dt)
    ...
    elseif not tTransientEventState.tDialogStatus.bRequestUI then
        ...
        local ignoreRefusal = math.random() > CompoundEvent.nChanceObey

        if tPersistentEventState.bMega then
            ignoreRefusal = false
        end
