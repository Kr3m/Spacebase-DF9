...
function GameRules.placeModule(tModuleData, tileXOff, tileYOff, bOnPlayerTeam)
    ...
    if tFakeEvent.bValidDockingData then
        Docking = require('Docking')
        local rEventClass = g_EventController.tEventClasses[tFakeEvent.sEventType]
        local tModule = Docking.getModule(tFakeEvent.sSetName,tFakeEvent.sModuleName)
        if tModule.tCrewSpawns then
            tFakeEvent.tCrewData = Event.spawnModuleCrew(tModule, rEventClass, tFakeEvent.nDifficulty)
        end
        if tModule.tObjectSpawns then
            tFakeEvent.tObjectData = Event.spawnModuleObjects(tModule)
        end
        tFakeEvent.bPrerolledSpawns = true
    end
