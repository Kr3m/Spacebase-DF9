...
function EmergencyBeacon:charWaiting(rChar,dt)
    if self:_testChar(rChar,'waited') then
...
function EmergencyBeacon:charAbandoned(rChar)
    if self:_testChar(rChar,'abandoned') then
...
function EmergencyBeacon:getBeaconForSquad(sSquadName)
    return self.tBeacons[sSquadName] or nil
end
