...
-- Grab melee weapon Cooldown data
function AttackEnemy:_getMeleeCooldown(rChar)
    local tWeapon = rChar.sDrawnWeapon and rChar.tInventory[rChar.sDrawnWeapon]
    if tWeapon then
        local nMeleeCoolDown = Inventory.getMeleeCooldown(tWeapon)
        if nMeleeCoolDown then
            return nMeleeCoolDown
        end
    end
end
...
function AttackEnemy:_attemptGrapple(dt)
    if self.rChar:isAdjacentToObj(self.rVictim) then
        ...
        local nMeleeCoolDown = self:_getMeleeCooldown(self.rChar)
        if nMeleeCoolDown then
            self.nAttackDuration =  nMeleeCoolDown
        else
...
function AttackEnemy:_attemptRangedAttack()
    ...
    if self:_lineOfSight(self.rChar,self.rVictim) then
        ...
        self.bShooting = true
        self.nShootingTimeElapsed = 0
        local nMaxCoolDown,nMinCoolDown,nMinAimTime,nMaxAimTime = self:_getGunCooldown(self.rChar)
        if nMaxCoolDown and nMinCoolDown and nMinAimTime and nMaxAimTime then
            self.nNextShootAimTime = DFMath.randomFloat(nMinAimTime,nMaxAimTime)
            self.nNextShootCooldownTime = DFMath.randomFloat(nMinCoolDown,nMaxCoolDown)
        else
...
-- Grab weapon Cooldown data
function AttackEnemy:_getGunCooldown(rChar)
    local tWeapon = rChar.sDrawnWeapon and rChar.tInventory[rChar.sDrawnWeapon]
    if tWeapon then
        local nMaxCoolDown,nMinCoolDown,nMinAimTime,nMaxAimTime = Inventory.getWeaponCooldown(tWeapon)
        if nMaxCoolDown and nMinCoolDown and nMinAimTime and nMaxAimTime then
            return nMaxCoolDown,nMinCoolDown,nMinAimTime,nMaxAimTime
        end
    end
end
