...
local tDiseaseSpecific = {'thing','Thing','Hyper','hyper','SocialWorm','socialWorm','firePlague','FirePlague'}
...
local tDiseaseAdjectives = {
    ...
    FirePlague = {
        'FIREADJECTIVE001', 'FIREADJECTIVE002',
        'FIREADJECTIVE003', 'FIREADJECTIVE004',
        'FIREADJECTIVE005', 'FIREADJECTIVE006',
        'FIREADJECTIVE007', 'FIREADJECTIVE008', },
}
...
function Malady._tickMalady(rChar,tMalady)
    ...
    if tMalady.bSymptomatic then
        ...
        --The Fire Plague
        if tMalady.sSpecial == 'fire' and (not tMalady.nNextSpawnAttempt or tMalady.nNextSpawnAttempt < GameRules.elapsedTime) then
            tMalady.nNextSpawnAttempt = GameRules.elapsedTime +  math.random(60,(60*5))
            if math.random(0,100) < 50 then
                rChar.catchFire(rChar)
            end
        end
...
function Malady._testSpread(tMalady, rSource,rTarget)
    --Using this for diseases,wormParisites, could easily expand to other types, eg parasites,viruses,bacteria, in fact I will do that
    if tMalady.sType=='Disease' then
        ...
        --Worm Parasites right now use the standard function, considering moving to specific "spreadTheWorm" task..
    elseif tMalady.sType=='WormParisite' then
        if not rTarget.isImmuneTo or not rTarget:isImmuneTo(tMalady) then
            local nInfectChance=tMalady.nChanceToInfect
            if ObjectList.getObjType(rTarget) == ObjectList.CHARACTER then
                if rTarget:getJob() == Character.DOCTOR then
                    --A doctor who is also a brain eating parasite thrall? infect all of the people!
                    nInfectChance = 1
                end
            else
                nInfectChance = 0
...
--Utility
function Malady.spawnThing(rChar)
    ...
    if g_Config:getConfigValue('disable_hostiles')  or not  rChar then
        return false
    end
    --Loop through the illness list to find a specific malady type
    local tIllList, nNum =  rChar:getIllnesses()
    sMName = ''
    ...
    if not  rChar:isDead() then
        local tLogData = {}
        --Kill and delete for "things".
        Log.add(Log.tTypes.DEATH_THING,  rChar, tLogData)
        CharacterManager.killCharacter(rChar, Character.CAUSE_OF_DEATH.THING)
    end
...
function Malady.spawnMonster(rChar)
    ...
    CharacterManager.killCharacter(rChar, Character.CAUSE_OF_DEATH.PARASITE)
    local nwx,nwy = rChar:getLoc()
    local tData = { tStats={ nRace = Character.RACE_MONSTER, sName = 'Parasite' } }
    CharacterManager.addNewCharacter(nwx,nwy,tData,Character.TEAM_ID_DEBUG_ENEMYGROUP)

    local tLogData = {}
    Log.add(Log.tTypes.DEATH_CHESTBURST, rChar, tLogData)
