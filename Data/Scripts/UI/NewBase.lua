...
function m.create()
    ...
    function Ob:setMapLoc()
        ...
        -- for stretchy map
        -- tiny scale bump to completely fill area
        local sw,sh = (targetWidth/mapWidth)+0.005, (sh/mapHeight)+0.001
    ...
    function Ob:onKeyboard(key, bDown)
        -- don't accept input while deploying
        if self.nDeployTime then
            return
        end
        -- ESC - bail out to pause screen
        if key == 27 and bDown then
            self:colorProp(self.rOverlay, Gui.BLACK_NO_ALPHA, Gui.BLACK_NO_ALPHA, 0, 0, nil)
            self:resume()
            g_GuiManager.setCursorVisible(true)
