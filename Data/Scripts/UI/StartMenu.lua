...
function m.create()
    ...
    function Ob:init()
		...
		self.rButtonLoadBase = self:getTemplateElement('ButtonLoadBase')
        self.rButtonLoadBase:addPressedCallback(self.loadBase, self)
		self.rButtonSaveBase = self:getTemplateElement('ButtonSaveBase')
        self.rButtonSaveBase:addPressedCallback(self.saveBase, self)
    ...
    function Ob:readMOTDTest()
        local sPath = DFFile.getDataPath("UILayouts/motd-client.json")
        ...
        if f~=nil then
            ...
            return '{"body":[{"text":"Could not connect to SpaceBaseHub.net nor find the local motd cache","y":70}],"footer":{"text": ">> SpaceBaseHub", "url": "http://www.spacebasehub.net/"}}'
        end
    end
    ...
    function Ob:refreshMOTD()
        ...
        --else
        local sMOTDURL = 'http://spacebasehub.net/motd.json'
	    self.rMOTDTask:setUserAgent("Spacebase DF-9 " .. MOAIEnvironment.appVersion .. " on " .. MOAIEnvironment.osBrand )
...
--[[
    ...
    function Ob:quit()
		...
		self.quit = require('UI.SaveYesNo').new() 
		Gui.setActivePane(self.quit)
    end

	function Ob:loadBase()
		SoundManager.playSfx('select')

        self.loadBase = require('UI.LoadBase').new()
        Gui.setActivePane(self.loadBase)
		--print('TEEEEEEEEEST - loadSave: -------  ' .. tostring(self.loadSave))
	end

	function Ob:saveBase()
		SoundManager.playSfx('select')

        self.saveBase = require('UI.SaveBase').new()
        Gui.setActivePane(self.saveBase)
		--print('TEEEEEEEEEST - loadSave: -------  ' .. tostring(self.loadSave))
	end
