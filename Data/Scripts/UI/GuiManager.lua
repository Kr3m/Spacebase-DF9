...
function GuiManager.showDGLogo()
	...
	local sTexturePath = 'Texture/Dgl.png'
	local nImageWidth, nImageHeight = 1280, 720
	local nImageScaleX, nImageScaleY = 1300, 1000
	local nPriority = 100
	local rMOAIGfxQuad2D = MOAIGfxQuad2D.new()
	rMOAIGfxQuad2D:setTexture(sTexturePath)
	if  not rMOAIGfxQuad2D then
		print('GuiManager.showDGLogo()', 'Failed to load texture', sTexturePath)
		return
	end
	rMOAIGfxQuad2D:setRect( { -nImageWidth / 2, -nImageHeight / 2, nImageWidth / 2, nImageHeight / 2 } )
	local rProp = MOAIProp2D.new()
	rProp:setDeck(rMOAIGfxQuad2D)
	rProp:setLoc(Renderer.getViewport().sizeX / 2, -Renderer.getViewport().sizeY / 2)
	rProp:setScl(nImageScaleX, nImageScaleY)
	Renderer.getRenderLayer('UIOverlay'):insertProp(rProp)
	rProp:setPriority(nPriority)
	rProp:setVisible(true)
	GuiManager.DGLogo = rProp
end

function GuiManager.hideDGLogo()
	Renderer.getRenderLayer('UIOverlay'):removeProp(GuiManager.DGLogo)
end
...
function GuiManager.setSelectionProp(object)
	...
	elseif object._ObjectList_ObjectMarker.objType == ObjectList.ENVOBJECT then
		...
		if objDef.SelectedObjectScale == nil then
			scale = objDef.width
		else
			scale = objDef.SelectedObjectScale
		end
		
		if objDef.SelectedObjectXoffset == nil then
			xoff = 0
		else
			xoff = objDef.SelectedObjectXoffset
		end
		
		if objDef.SelectedObjectYoffset == nil then
			yoff = 0
		else
			yoff = objDef.SelectedObjectYoffset
		end
		--print("GUIMANAGER.LUA: objname : " .. object.sName .. "  xoffset: " .. objDef.SelectedObjectXoffset .. " yoffset: " .. objDef.SelectedObjectYoffset)
