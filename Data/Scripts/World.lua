...
function World.getPerpendicularDirection(d)
    if d == World.directions.SAME then
        return World.directions.SAME
    elseif d == World.directions.NW then
        return World.directions.NE
...
function World.getIndoorTarget(nRadius, bRequireSafeAndPathable)
    local Docking = require('Docking')
    local tRooms = Docking.getPlayerRooms(bRequiresSafeAndPathable, false)
    ...
    if rSelectedRoom then
        ...
        if tx and not bRequireSafeAndPathable then
            ...
            wx, wy = MiscUtil.randomPointWithinRadius(wx, wy, nRadius * 0.5)
    ...
    local nBufferInTiles = math.ceil(nRadius / 128) + 1
