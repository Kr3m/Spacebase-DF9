...
-- returns 0-1
function Character:getBaseJobCompetency(nJob)
    -- starting competency is 0-2
    -- level is 1-10
    -- but max competency should be awarded at a combined value of 10.
    local competency = 0
    if self.tStats.tJobCompetency[nJob] ~= nil then
        competency = self.tStats.tJobCompetency[nJob] + self:getCurrentLevelByJob(nJob)
...
--Grab weapon in inventory with the highest "nDmg" value, may want to make smarter
...
--This is what spawns weapons from thin air for security  officers when they do not have one.
...
function Character:_shouldAttackLethal(rTarget)
    ...
    if self.tStats.nJob == Character.EMERGENCY then
        ...
        -- brawlers never try to kill their opponent (consider changing)
...
function Character:_shouldRespondToTantrum()
    ...
    if rChar and rChar ~= self and not rChar:isDead() and rChar.tStatus.bRampageNonviolent and not Malady.isIncapacitated(self) then
...
function Character:shouldTargetForAttack(rTarget)
    ...
    if self.tStats.nJob == Character.EMERGENCY and rTarget.tStatus.bMarkedForExecution then
        return true
    elseif g_ERBeacon:getViolence(self.squadName) == g_ERBeacon.VIOLENCE_NONLETHAL and Malady.isIncapacitated(rTarget) then
        return false
        --If target is rampaging violently and you are security, attack them.
    elseif self.tStats.nJob == Character.EMERGENCY and rTarget:hasUtilityStatus(Character.STATUS_RAMPAGE_VIOLENT) then
        return true
    end
    ...
    if self:isBrawling(rTarget) then
        -- stop brawling immediately if in space, because why would you fight someone when both of you have a spacesuit on, stop brawling, don't kill em!
        local tileValue = World.getTileValueFromWorld(self:getLoc())
        if tileValue == World.logicalTiles.SPACE or bIncapacitated or rTarget:isDead() then
...
--grab best weapon in inventory and grab its range, [TODO] may want to fall back on a melee weapon when the are too close.
...
function Character:shootAt(rVictim, sAttachJointName, tOffset)
    ...
    if tWeapon then
        local _,_,_,_,sBulletSprite = Inventory.getWeaponData(tWeapon)
        if sBulletSprite then
            sSpriteName = sBulletSprite
        end
    end
...
function Character:_applyDrugs(tx,ty)
    if self.tStats.nTickDrugged and self.tStats.nTickDrugged > 0 then
        self.tStats.nTickDrugged = self.tStats.nTickDrugged-1
        return;
    end
    local tDruggers = require('EnvObjects.EnvObject').getObjectsOfType('HappyBot', true, true)
    for _,rDrugger in pairs(tDruggers) do
        -- only active, powered scrubbers work
        if rDrugger:isFunctioning() and MiscUtil.isoDist(tx,ty,rDrugger:getTileLoc()) <= rDrugger.tData.nRange then
            --1 minute in between attempts
            self.tStats.nTickDrugged=2
            if not self:getHasMaladyOfTier(-2) then
                self:diseaseInteraction(nil,Malady.createNewMaladyInstance('Drugged'))
            end
        end
    end
end
...
function Character:catchFire(rChar)
    if not self then
        self = rChar
    end
...
function Character:_setStats( tData )
        ...
        if not self.tStats.nTickDrugged then self.tStats.nTickDrugged = 2 end
        --done
...
function Character:spacesuitOn(bInitializing)
        ...
        --Apply SpaceSuit Tech
         if Base.hasCompletedResearch('SpaceSuit2') then
            self.tStatus.suitOxygen = 600*Character.OXYGEN_PER_SECOND
         end
        end
...
function Character:getObjectAffinity(tItem,bNormalize)
    ...
    local job,bJobTool = Inventory.getItemJob(tItem)
    if bJobTool and job ~= Character.EMERGENCY then
    ...
    elseif job == Character.EMERGENCY then
        --Encourage people to keep weapons on them ( a weapon is any item tagged as a security item for now),in case raiders, might add a personality facet for it
        local nAff=1
        local nTags = 0;
            for k,v in pairs(InventoryData.tTags) do
                nTags = nTags+1
            ...
            if math.random(100) < 75 then
               self.tAffinity[tItem.sName] = nAff+math.random(15,Character.MAX_AFFINITY)
            else
               self.tAffinity[tItem.sName] = nAff+math.random(1,10)
            end
...
--Code that allows people to drop items.
...
function Character:_factionSetup(bLoading)
    ...
    elseif nBehavior == Character.FACTION_BEHAVIOR.EnemyGroup then
        ...
        if self:getJob() == Character.RAIDER and self.tStats.nChallengeLevel then
            ...
            -- Toughness: 0-1
            -- Point buy!
            --Note this change will make raiders much tougher or much weaker to deal with, will require balancing
            local tWeapons={'Melee','Pistol','AutoPistol','LaserRifle','PlasmaRifle','RedLaserRifle','RedPistol','SniperRifle'}
            local tMeleeWeapons={'none','VibroKnife','PunchingGloves','Rey5w0rd','CryoVise'}
            local tOptions={weapon=false ,armor=false ,secondaryWeapon=false }
            local gunCost=6
            local nPoints=math.min(8, math.floor(self.tStats.nChallengeLevel * 25 + .5))

            self.tStats.nToughness = 0

            while nPoints > 0 do

                if math.random(0,50)>25 and self.tStats.nToughness < 1 and  nPoints > 0 then
                    nPoints = nPoints-1
                    self.tStats.nToughness =  self.tStats.nToughness + .1
                end

                if math.random(0,50)>25 and tOptions.armor==false and  nPoints > 0 then
                    armorPoints = math.random(4)-1
                    nPoints=nPoints-armorPoints
                    tOptions.armor=true
                    self:pickUpItem(Inventory.createItem('ArmorLevel'..armorPoints))
               end

                if math.random(50)<25 and tOptions.weapon==false and  nPoints > 0 then
                    local weapon =  math.random(1,#tWeapons)
                    local tWeapon = { sTemplate = tWeapons[weapon] }
                    local cost=0
                     if tWeapons[weapon] ~= 'Melee' then
                        cost = (gunCost + Inventory.getWeaponPoints(tWeapon))
                     end
                    if tWeapons[weapon] ~= 'Melee' and Inventory.getWeaponPoints(tWeapon) and nPoints >= cost  then
                        nPoints= (nPoints-cost)
                        self:pickUpItem(Inventory.createItem(tWeapons[weapon]))
                        tOptions.weapon=true
                    end
                    if tWeapons[weapon]=='Melee' then
                        tOptions.weapon=true
                    end
                ...
                if math.random(0,3)==2 and tOptions.secondaryWeapon==false and  nPoints > 0 then
                    secondaryWeapon = math.random(1,#tMeleeWeapons)
                    if tMeleeWeapons[secondaryWeapon] ~= 'none' then
                        nPoints=nPoints-(secondaryWeapon+2)
                        self:pickUpItem(Inventory.createItem(tMeleeWeapons[secondaryWeapon]))
                    end
                    tOptions.secondaryWeapon=true
              end

              if tOptions.secondaryWeapon==true and tOptions.weapon==true and tOptions.armor==true and self.tStats.nToughness >= 1 then
                nPoints=0
              end
...
function Character:infestFromObject(rSource, sDiseaseName)
        --I am creating this for other maladies we might want to infect people with, like a zombie infestation for example.
...
function Character:getHealth()
    ...
    if self.tStats.nRace ~= Character.RACE_KILLBOT then
        if self:getHasMaladyOfTier(-2) then
            return Character.STATUS_DRUGGED
        end
    end
