...
function Docking.lineWorldBoundsIntersect(x1,y1,x2,y2, nMinDist)
...
function Docking._spawnModule(tEventData, tModuleData, tx, ty, tDockingTile)
    ...
    if tDockingTile then
        ...
        local nLeftDirection = g_World.getPerpendicularDirection(nEmptySpaceDirection)
...
function Docking.getPlayerRooms(bSafeOnly, bExternalOnly)
    local tRooms = Room.getRoomsOfTeam(Character.TEAM_ID_PLAYER)
    local tSelectedRooms = {}
    for rRoom,id in pairs(tRooms) do
        local bUseRoom = true

        if bSafeOnly and rRoom:isDangerous() then
            bUseRoom = false
        end

        if bExternalOnly and not rRoom.bExterior then
            bUseRoom = false
        end

        if bUseRoom then
            tSelectedRooms[id] = rRoom
        end
    end
    return tSelectedRooms
end
...
function Docking.attemptQueueEvent(sEventType, nEventDifficulty)
    ...
    local tExternalRooms = Docking.getPlayerRooms(true, true)
    local nExternalRooms = DFUtil.tableSize(tExternalRooms)
    local tRooms = Room.getRoomsOfTeam(Character.TEAM_ID_PLAYER)
    local nRooms = DFUtil.tableSize(tRooms)

    -- no rooms! OR, if this is a docking event and no exterior rooms
    if nRooms == 0 or (tEventRequirements and tEventRequirements.bDocking and nExternalRooms < 1) then
        return
    end
    ...
    for name,event in pairs(ModuleData[sEventType]) do
        ...
        elseif event.tRequiredZones then
            ...
            for i,room in pairs(tExternalRooms) do
