...
function Inventory.getWeaponData(tItem)
    ...
    if tItemTemplate then
        return tItemTemplate.nDamage,tItemTemplate.nDamageType,tItemTemplate.nRange,tItemTemplate.sStance,tItemTemplate.sBulletSprite
    end
end

--Function was getting clunky so added new one specifically for getting cooldowndata
function Inventory.getWeaponCooldown(tItem)
    local tItemTemplate = InventoryData.tTemplates[tItem.sTemplate]
    if tItemTemplate then
        return tItemTemplate.nMaxCoolDown,tItemTemplate.nMinCoolDown,tItemTemplate.nMinAimTime,tItemTemplate.nMaxAimTime
    end
end

--Seperate function for melee cooldowns
function Inventory.getMeleeCooldown(tItem)
    local tItemTemplate = InventoryData.tTemplates[tItem.sTemplate]
    if tItemTemplate then
        return tItemTemplate.nMeleeCoolDown
    end
end

--Grab the point value of a weapon
function Inventory.getWeaponPoints(tItem)
    local tItemTemplate = InventoryData.tTemplates[tItem.sTemplate]
    if tItemTemplate then
        return tItemTemplate.nPoints
    end
end
...
function Inventory.getAffinityDecay(tItem)
    ...
    if InventoryData.tTemplates[tItem.sTemplate].nDamage then
        return InventoryData.DEFAULT_AFFINITY_DECAY*.75
    end
