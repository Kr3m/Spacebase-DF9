...
-- Tests if a block is obstructed for pathfinding
-- Returns true is obstructed, false otherwise.
function Door:_testObstruction(tx,ty)
    local bIsObstructed = g_World._shouldObstructPathing(tx,ty,g_World._getTileValue(tx,ty))
    return (bIsObstructed or false)
end
...
function Door:_updateSpaceStatus()
    ...
    self.bWestSideObstructed,self.bEastSideObstructed=false,false
    self.bIsObstructed = false
    ...
    for side=1,2 do
	local tSide = (side == 1 and self.tWestSideTiles) or self.tEastSideTiles
	local bObstructed = false
	for i,addr in ipairs(tSide) do
	    local tx, ty = g_World.pathGrid:cellAddrToCoord(addr)
	    if self:_testObstruction(tx,ty) then
	        bObstructed=true
	    end
	    if side == 1 then
		self.bWestSideObstructed = bObstructed
	    else
		self.bEastSideObstructed = bObstructed
	    end
	end
     end
     self.bIsObstructed = (self.bWestSideObstructed or self.bEastSideObstructed)
end
...
function Door:_updateDoorState(bForceToOperation)
    ...
    elseif self.operation == Door.operations.FORCED_OPEN then
        ...
        self:_setDoorState(Door.doorStates.OPEN)
    elseif self:_isSabotaged() or self.operation == Door.operations.LOCKED or (self.bTouchesVacuum and self.bEastSideVacuum ~= self.bWestSideVacuum) or self.bIsObstructed then
    ...
    elseif self.operation == Door.operations.NORMAL then
		...
		if not self:hasPower() then
			if self.bTouchesVacuum and self.bEastSideVacuum ~= self.bWestSideVacuum then
				...
				self:_setDoorState(Door.doorStates.LOCKED)
			elseif self.bIsObstructed then
				if self.doorState == Door.doorStates.LOCKED then
					Profile.leaveScope("Door_State")
					return
				end
