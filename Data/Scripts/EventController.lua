...
EventController.nEventForecastMax = 10
-- number of previous events to retain details of in the save file
EventController.nPrevEventsCount = 5

-- final siege time
EventController.nFinalSiegeTime = 60 * 60 * 8
-- Cap playtime with regard to difficulty and various curves
...
function EventController.setBaseSeeds()
    ...
    for _, rClass in pairs(EventController.tEventClasses) do
        ...
        local nWeight
        if rClass.sEventType == 'immigrationEvents' or rClass.sEventType == 'CompoundEvent' or rClass.sEventType == 'breachingEvents' then
            nWeight = rClass.getWeight(25, 60*60*2, true)
        else
            nWeight = rClass.nDefaultWeight
        end
...
function EventController.generateEventForecast(nForcePop,nForceTime)
    ...
    for i=1,EventController.nEventForecastMax do
        ...
        for sEventType, rClass in pairs(EventController.tEventClasses) do
            ...
            and not (rClass.bHostile and g_Config:getConfigValue('disable_hostiles'))
            and not (rClass.bHostile and g_Config:getConfigValue('disable_hostile_events'))
            and (nPopulationEstimate > rClass.nMinPopulation or GameRules.elapsedTime > rClass.nMinTime) then

                if (rClass.nMaxPopulation >= 0 and nPopulationEstimate >= rClass.nMaxPopulation) then
                    weight = 0
                elseif (rClass.nMinPopulation >= 0 and nPopulationEstimate < rClass.nMinPopulation) then
                    weight = 0
                elseif (rClass.nMaxUndiscoveredRooms >= 0 and nHiddenRooms >= rClass.nMaxUndiscoveredRooms) then
                    weight = 0
                elseif (rClass.nMaxExteriorRooms >= 0 and nExteriorRooms >= rClass.nMaxExteriorRooms) then
                    weight = 0
                elseif (rClass.nMinExteriorRooms >= 0 and nExteriorRooms < rClass.nMinExteriorRooms) then
                    weight = 0
                elseif rClass.sEventType == 'immigrationEvents' or rClass.sEventType == 'CompoundEvent' or rClass.sEventType == 'breachingEvents' then
                    weight = rClass.getWeight(nPopulationEstimate, nTimeEstimate)
                else
                    weight = rClass.nDefaultWeight
                end
